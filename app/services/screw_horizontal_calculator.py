...
class ScrewHorizontalCalculator:
    """丝杠水平运动选型计算器"""
    
    SCENARIO_NAMES = {
        "screw_horizontal": "丝杠水平运动选型计算",
    ...
    def calculate(self, scenario: str, params: Dict[str, Any]) -> CurrentCalcResponse:
        ...
        """
        if scenario == "screw_horizontal":
            return self._calculate_screw_horizontal(params)
    ...
    def _calculate_load_torque(self, params: Dict[str, Any]) -> CurrentCalcResponse:
        ...
        formula = f"轴向负载: F = F<sub>A</sub> + M×G×(sin a + μ×cos a) = {FA} + {M}×{self.G}×(sin({a}°) + {mu}×cos({a}°)) = {F:.4f} N<br>"
        formula += f"负载转矩: T<sub>L</sub> = (F × P<sub>B</sub>) / (2π × η) = ({F:.4f} × {PB}) / (2π × {eta}) = {TL:.6f}"
        
        # 使用formula字段存储额外信息（F值）
        # 注意：这里我们无法直接返回extra字段，但可以在formula中包含F值
        # 前端可以通过解析formula来提取F值，或者我们可以在响应中添加extra字段
    ...
    def _calculate_acceleration_torque(self, params: Dict[str, Any]) -> CurrentCalcResponse:
        ...
        formula = f"直线运动平台惯量: J<sub>L1</sub> = M × (P<sub>B</sub>/(2π))² = {M} × ({PB}/(2π))² = {JL1:.6f} kg·m²<br>"
        formula += f"滚珠丝杠惯量: J<sub>B</sub> = (π/32) × ρ × L<sub>B</sub> × D<sub>B</sub>⁴ = (π/32) × {self.RHO} × {LB} × {DB}⁴ = {JB:.6f} kg·m²<br>"
        formula += f"连轴器惯量: J<sub>C</sub> = M<sub>C</sub> × D<sub>C</sub>² / 8 = {MC} × {DC}² / 8 = {JC:.6f} kg·m²<br>"
        formula += f"总负荷惯量: J<sub>L</sub> = J<sub>L1</sub> + J<sub>B</sub> + J<sub>C</sub> = {JL1:.6f} + {JB:.6f} + {JC:.6f} = {JL:.6f} kg·m²<br>"
        formula += f"启动转矩: T<sub>S</sub> = (2π × N<sub>M</sub> × (J<sub>M</sub> + J<sub>L</sub>)) / (60 × t<sub>0</sub>) = (2π × {NM} × ({JM} + {JL:.6f})) / (60 × {t0}) = {TS:.6f}"
    ...
    def _calculate_inertia_ratio_reducer(self, params: Dict[str, Any]) -> CurrentCalcResponse:
        ...
        # 调试信息
        import logging
        logging.info(f"_calculate_inertia_ratio_reducer 接收参数: JL={JL}, JM={JM}, i={i}, params={params}")
        
        if JL is None:
            raise ValueError("总负荷惯量JL不能为空")
        try:
            JL = float(JL)
        except (ValueError, TypeError):
            raise ValueError(f"总负荷惯量JL必须是有效数字，当前值: {JL}")
        if JL <= 0:
            raise ValueError(f"总负荷惯量JL必须大于0，当前值: {JL}")
            
        if JM is None:
            raise ValueError("电机惯量JM不能为空")
        try:
            JM = float(JM)
        except (ValueError, TypeError):
            raise ValueError(f"电机惯量JM必须是有效数字，当前值: {JM}")
        if JM <= 0:
            raise ValueError(f"电机惯量JM必须大于0，当前值: {JM}")
            
        if i is None:
            raise ValueError("减速机减速比i不能为空")
        try:
            i = float(i)
        except (ValueError, TypeError):
            raise ValueError(f"减速机减速比i必须是有效数字，当前值: {i}")
        if i <= 0:
            raise ValueError(f"减速机减速比i必须大于0，当前值: {i}")
    ...
    def _calculate_screw_horizontal(self, params: Dict[str, Any]) -> CurrentCalcResponse:
        """丝杠水平运动选型计算（完整计算）"""
        # 输入参数
        ...
        a = params.get("a", 0)  # 移动方向与水平轴夹角 (°)
        S = params.get("S", 1)  # 安全系数
        JM = params.get("JM", 0.0011)  # 电机惯量 (kg·m²)
        ...
        # 构建详细公式字符串
        formula = f"计算步骤：\n"
        formula += f"1) 加速时间: t0 = t × A = {t} × {A} = {t0:.4f} s\n"
        formula += f"2) 电机转速: NM = Vl / PB = {Vl} / {PB} = {NM:.2f} rpm\n"
        formula += f"3) 轴向负载: F = FA + M×G×(sin(a) + μ×cos(a)) = {FA} + {M}×{self.G}×(sin({a}°) + {mu}×cos({a}°)) = {F:.4f} N\n"
        formula += f"   负载转矩: TL = (F × PB) / (2π × η) = ({F:.4f} × {PB}) / (2π × {eta}) = {TL:.4f} Nm\n"
        formula += f"4) 直线运动平台惯量: JL1 = M × (PB/(2π))² = {M} × ({PB}/(2π))² = {JL1:.6f} kg·m²\n"
        formula += f"   滚珠丝杠惯量: JB = π × ρ × LB × DB⁴ / 32 = π × {self.RHO} × {LB} × {DB}⁴ / 32 = {JB:.6f} kg·m²\n"
        formula += f"   连轴器惯量: JC = MC × DC² / 8 = {MC} × {DC}² / 8 = {JC:.6f} kg·m²\n"
        formula += f"   总负荷惯量: JL = JL1 + JB + JC = {JL1:.6f} + {JB:.6f} + {JC:.6f} = {JL:.6f} kg·m²\n"
        formula += f"   启动转矩: TS = 2π × NM × (JM + JL) / (60 × t0) = 2π × {NM:.2f} × ({JM} + {JL:.6f}) / (60 × {t0:.4f}) = {TS:.4f} Nm\n"
        formula += f"5) 必须转矩: TM = (TL + TS) × S = ({TL:.4f} + {TS:.4f}) × {S} = {TM:.4f} Nm\n"
        formula += f"7) 惯量比: I1 = JL / JM = {JL:.6f} / {JM} = {I1:.2f}\n"
        ...
        return CurrentCalcResponse(
            ...
            unit="Nm",
            formula=formula,
            scenario_name=self.SCENARIO_NAMES["screw_horizontal"],
        )
